...
void TIM2_Init(unsigned int arr,unsigned int psc)
{
    ...
    __HAL_TIM_DISABLE(&TIM2_Handler);//禁止中断
}

//==========================================================
//	函数名称：	void TIM2_Delay10US(uint32_t nscount)
//
...
void TIM2_Delay10US(uint32_t nscount)
{
    ...
    TIM2->CNT = 0;
    while(__HAL_TIM_GetCounter(&TIM2_Handler)<nscount);
...
//==========================================================
//	函数名称：	void TIM_DelayUS(uint32_t nscount)
//
//	函数功能：	实现以定时器3为基准的微秒级延时。
//
//	入口参数：	
//
//	返回参数：	无
...
//	说明：			无
//==========================================================
void TIM3_DelayUS(uint32_t nscount)
{
    __HAL_TIM_ENABLE(&TIM3_Handler);
    //TIM3->CR1 |= TIM_CR1_CEN;
    //__HAL_TIM_SetCounter();
    TIM3->CNT = 0;
    while(__HAL_TIM_GetCounter(&TIM3_Handler)<nscount);
