...
unsigned char  Send_Data[Send_REC_LEN]; 			//发送缓冲,最大Send_REC_LEN个字节
unsigned char  Receive_Data[Receive_REC_LEN]; //接收缓冲,最大Send_REC_LEN个字节
...
void Rs485_Init(void)
{
    ...
    __HAL_RCC_GPIOA_CLK_ENABLE();           	//开启GPIOA时钟
		...
		HAL_GPIO_WritePin(GPIOA,GPIO_PIN_8,GPIO_PIN_RESET);	//PA8置0   初始化为接收模式
...
void Tx_Stack_Init(void)
{
	...
	Tx_Stack.Src_Adr = 0x00;						//源地址
	Tx_Stack.Dst_Adr = 0x00;						//目的地址
...
void Rx_Stack_Init(void)
{
	...
	Rx_Stack.Src_Adr = 0x00;						//源地址
	Rx_Stack.Dst_Adr = 0x00;						//目的地址
...
{
	static uint16_t i = 0;															//清空数组时For循环使用	
	...
	Tx_Stack.Pack_Num++;
	Send_Data[5] = SrcAdr;															//源地址
	Send_Data[6] = DstAdr;															//目的地址
	Send_Data[7] = Cmd>>8;
	Send_Data[8] = Cmd;																	//命令码
	Send_Data[9] = Len;																	//数据长度
	for(i=10;i<(Len+10);i++)
	{
		Send_Data[i] = pData[i-10];												//数据
	}
	Tx_Stack.CrcCheck = CRC_16(Send_Data,(10+Len));			//CRC校验码
	Send_Data[10+Len] = Tx_Stack.CrcCheck>>8;
	Send_Data[11+Len] = Tx_Stack.CrcCheck;							//CRC校验码
	for(i=(12+Len);i<(16+Len);i++)
	{
		Send_Data[i] = (0xEA+(i-12-Len));									//帧尾
	}

	RS485_TX_EN();		//开启485发送
	delay_ms(5);
	HAL_UART_Transmit(&UART1_Handler,Send_Data,(16+Len),1000);//485发送数据
	delay_ms(5);
	RS485_RX_EN();		//开启485接收
}
...
uint8_t DataHandling_485(uint8_t DesAddr)
{
  ...
  if(USART1_RX_STA == 0x8000)
	{		
			Return_Value = 0;
			if(Receive_Data[6] == DesAddr)	//对比源地址是想要的数据，全部接收下来
			{
				/*包序号*/
