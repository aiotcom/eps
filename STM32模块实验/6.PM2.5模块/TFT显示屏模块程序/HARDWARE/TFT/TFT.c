...
void LCD_UI(void)
{
	...
	LCD_ShowString(90,30,"P M 2.5",1);
	LCD_DrawRectangle(1,60,239,150);			//数据显示框
	LCD_ShowString(6,80,"Value(ug/m3):",1);
}
...
{
	static uint8_t PMData[10];
	static uint16_t pmdata = 0;
	
	if(!DataHandling_485(Addr_LCD))		//接收数据
	{
		pmdata = Rx_Stack.Data[0]*256 + Rx_Stack.Data[1];
		sprintf((char *)PMData,"Value:%d ug/m3  ",pmdata);
		LCD_ShowString(6,80,PMData,0);	
		memset(PMData,0,10);
	}
}
