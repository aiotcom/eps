...
#if (HAL_UART_DMA == 1)
  #define HAL_UART_PERCFG_BIT2        0x01         // USART0 on P0, Alt-1; so clear this bit.
  #define HAL_UART_Px_RX_TX2          0x0C         // Peripheral I/O Select for Rx/Tx.
  #define HAL_UART_Px_RTS            0x20         // Peripheral I/O Select for RTS.
  #define HAL_UART_Px_CTS            0x10         // Peripheral I/O Select for CTS.
#else
  #define HAL_UART_PERCFG_BIT2        0x02         // USART1 on P1, Alt-1; so set this bit.
  #define HAL_UART_Px_RTS            0x20         // Peripheral I/O Select for RTS.
  #define HAL_UART_Px_CTS            0x10         // Peripheral I/O Select for CTS.
  #define HAL_UART_Px_RX_TX2          0x30         // Peripheral I/O Select for Rx/Tx.
#endif
...
#if (HAL_UART_DMA == 1)
  PERCFG &= ~HAL_UART_PERCFG_BIT2;    // Set UART0 I/O to Alt. 1 location on P0.
#else
  PERCFG &= ~HAL_UART_PERCFG_BIT2;     // Set UART1 I/O to Alt. 2 location on P1.
#endif
  PxSEL  |= HAL_UART_Px_RX_TX2;       // Enable Tx and Rx on P1.
  ADCCFG &= ~HAL_UART_Px_RX_TX2;      // Make sure ADC doesnt use this.
