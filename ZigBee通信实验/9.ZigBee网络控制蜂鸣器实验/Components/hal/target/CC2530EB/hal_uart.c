...
初始化UART
*/
void UartInit(uint8 port,uint8 baud)
{
...
/*
*/
void UART_PrintData(uint8 *pprompt,uint32 data)
{
  uint8 temp[4]={0},temp1[4]={0},index=0,i = 0;
  HalUARTWrite(0,pprompt,strlen((char const *)pprompt));
  
  temp[index++] = (data&0x0F) + 0x30;
  data          = data >> 4;
  temp[index++] = (data&0x0F) + 0x30; 
  data          = data >> 4;
  temp[index++] = (data&0x0F) + 0x30;
  ...
  while(i<index)
  {
    if(temp[i]>0x39)
    {
      temp[i] = 0x41+temp[i]-0x39-1;
    }
    temp1[3-i] = temp[i];
    i++;
  }
  HalUARTWrite(0,temp1,index);
  HalUARTWrite(0,"\r\n",2);
}
/*
*/
uint8 uart_printstring_buf[50],uart_printstring_idx=0;
void UART_PrintBuffer(uint8 *pprompt,uint8 *pdata,uint16 len)
{
  uint8 temp,i = 0;
  HalUARTWrite(0,pprompt,strlen((char const *)pprompt));
  uart_printstring_idx = 0;
  
  
  while(len--)
  {
    temp = *(pdata++);
    uart_printstring_buf[uart_printstring_idx++] = ((temp>>4)&0x0F) + 0x30; //发高的四字节
    uart_printstring_buf[uart_printstring_idx++] = (temp&0x0F)      + 0x30;//发低的四字节
    uart_printstring_buf[uart_printstring_idx++] = ' ';  
  }
  
  while(i < uart_printstring_idx)
  {
    if(uart_printstring_buf[i] > 0x39)
    {
      uart_printstring_buf[i] = 0x41+uart_printstring_buf[i]-0x39-1;
    }
    i++;
  }
  
  HalUARTWrite(0,uart_printstring_buf,uart_printstring_idx);
  HalUARTWrite(0,"\r\n",2);
}

void UART_PrintString(uint8 *pdata)
{
  HalUARTWrite(0,pdata,strlen((char const *)pdata));
  HalUARTWrite(0,"\r\n",2);
}
