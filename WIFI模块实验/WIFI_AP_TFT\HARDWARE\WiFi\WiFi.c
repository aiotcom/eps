...
*/    
    ...
    while(!WiFi_Send_Cmd(CWSAP,(uint8_t*)"OK",3000));				/*配置要连接的路由器的SSID和密码*/   
    //WS2812B_SetColour(0x005500);                                    //红色提示    
    
    
    while(!WiFi_Send_Cmd(CIFSR,(uint8_t*)"OK",60));	/*与服务器建立TCP连接*/
    //WS2812B_SetColour(0x550000);                                    //绿色提示
    
    while(!WiFi_Send_Cmd(CIPMUX,(uint8_t*)"OK",60));				/*透传模式*/

    
    while(!WiFi_Send_Cmd(CIPSERVER,(uint8_t*)"OK",30));				/*开始传输*/

    while(!WiFi_Send_Cmd(CIPAPMAC,(uint8_t*)"OK",30));				/*开始传输*/
    
    while(!WiFi_Send_Cmd(CIPAP,(uint8_t*)"OK",30));				/*连接设备*/
