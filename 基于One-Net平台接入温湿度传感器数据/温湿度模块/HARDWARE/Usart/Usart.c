...
void Display_Data(void)
{
	...
	LDR_Data = Get_Adc_Average(ADC_CHANNEL_0,50);  		//获取光照强度
	Temp[0] = g_sht2x_param.TEMP_HM;		//温度数据
	Humi[0] = g_sht2x_param.HUMI_HM;		//湿度数据
	Light[0] = LDR_Data>>8;							//光强数据
	Light[1] = LDR_Data;								//光强数据
	Sum_Data[0] = g_sht2x_param.TEMP_HM;//温度数据
	Sum_Data[1] = g_sht2x_param.HUMI_HM;//湿度数据
	Sum_Data[2] = LDR_Data>>8;					//光强数据
	Sum_Data[3] = LDR_Data;							//光强数据
