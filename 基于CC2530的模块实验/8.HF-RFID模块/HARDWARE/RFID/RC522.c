...
unsigned char ucArray_ID[10];	//先后存放IC卡的类型和UID(IC卡序列号)
unsigned char SendData[50];     //SendData
unsigned char SendBuf[50];      //SendBuf
unsigned char Block_Date[20];	//HF_RFID-Data
unsigned char Data_Send[30];	//HF_RFID-ID+Data
  ...
  else
  {
    switch(data)
    {
      ...
      default:
        if(flag)
        {
          SendBuf[num] = (Block_Date[(num/2)]>>4)+0x30;
        }
        else
        {
          SendBuf[num] = (Block_Date[(num/2)]&0x0f)+0x30;
        }     
      break;
    }
  }
